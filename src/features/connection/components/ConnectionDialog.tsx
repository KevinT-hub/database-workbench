...
export const ConnectionDialog: React.FC<ConnectionDialogProps> = ({
  ...
  const { isTesting, testResult, resetTestResult, handleTestConnection } = useConnectionTest();
  ...
  const [idleTimeoutSeconds, setIdleTimeoutSeconds] = useState(28800); // 空闲超时，默认 28800 秒（8小时）
  ...
  useEffect(() => {
    if (isOpen) {
      if (editProfile) {
        ...
        setIdleTimeoutSeconds(editProfile.timeout || 28800); // 空闲超时默认 28800
        ...
        setIdleTimeoutSeconds(28800); // 空闲超时默认 28800 秒（8小时）
  ...
  }, [isOpen, editProfile, resetTestResult]);
  ...
  const buildProfile = (): ConnectionProfile => {
    ...
    return {
      ...
      charset,
      collation,
      timeout: idleTimeoutSeconds, // 空闲超时
  ...
  const onTestConnection = () => {
    ...
    void handleTestConnection(buildProfile());
  };
    ...
    <div className="connection-dialog-tab-content">
      ...
      <FormGroup label={t('dialog.connection.timeout')} labelFor="timeout">
        <InputGroup
          ...
          onChange={(e) => setIdleTimeoutSeconds(parseInt(e.target.value) || 28800)}
        />
      </FormGroup>
    </div>
  );
      ...
      >
        ...
        <div className={Classes.DIALOG_FOOTER}>
          <div className={Classes.DIALOG_FOOTER_ACTIONS}>
            ...
            <Button onClick={onTestConnection} loading={isTesting} disabled={isTesting}>
