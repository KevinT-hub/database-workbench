...
import { UpdaterService, type UpdateInfo, type DownloadProgress } from '@/features/updater/updaterService';
  ...
  const renderContent = () => {
    switch (status) {
      case 'checking':
        return (
          <div className="flex flex-col items-center justify-center gap-4 px-5 py-10">
            <Spinner size={32} />
            <p className="m-0 text-app-text opacity-70">{t('update.checking', '正在检查更新...')}</p>
          </div>
        );
            ...
            <Callout intent="primary" icon="download">
              ...
              {updateInfo?.date && (
                <p className="m-0 text-xs opacity-80">
                  {t('update.releaseDate', '发布日期')}: {formatReleaseDate(updateInfo.date)}
                </p>
              )}
            </Callout>
            {updateInfo?.body && (
              <div className="mt-4 max-h-[200px] overflow-y-auto rounded-md bg-[rgba(128,128,128,0.08)] p-3">
                <h4 className="m-0 mb-2 text-[13px] font-semibold">{t('update.releaseNotes', '更新说明')}</h4>
                <div>
                  {updateInfo.body.split('\n').map((line, index) => (
                    <p key={index} className="my-1 text-xs leading-[1.5]">{line}</p>
                  ))}
                </div>
              </div>
            )}
      ...
      case 'downloading':
        return (
          <div className="py-4">
            <p className="m-0 mb-3">{t('update.downloading', '正在下载更新...')}</p>
            <ProgressBar
            ...
            />
            {progress && (
              <p className="mt-2 text-center text-xs text-app-text opacity-70">
